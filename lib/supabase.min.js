/**
 * geoarmy-v2/lib/supabase.min.js
 *
 * Copia local, FIJADA (pinned), de @supabase/supabase-js -- reemplaza el
 * CDN externo con "@2" (etiqueta mutable) que usaban previamente
 * auth/x-complete/ y auth/callback/. Servida desde el mismo origen
 * (https://geoarmy.tv/geoarmy-v2/lib/supabase.min.js), asi que cargarla
 * ya no depende de que jsdelivr este disponible ni de confiar en que el
 * contenido detras de "@2" no cambie entre una visita y otra.
 *
 * Version exacta pineada: 2.115.0 -- la version que "@supabase/supabase-js@2"
 * resolvia en jsdelivr al momento de esta entrega (2026-09-05). Contenido:
 * dist/umd/supabase.js del paquete npm de esa version (el mismo build UMD
 * que geoarmy-account.js ya usaba via CDN -- define window.supabase con
 * .createClient(...), sin cambiar nada de como se usa esa API).
 *
 * Integridad verificada de forma cruzada y adversarial contra DOS fuentes
 * independientes al momento de fijar esta copia (no se confio en una sola
 * descarga de un solo CDN):
 ...
 * Ambas fuentes devolvieron EXACTAMENTE el mismo contenido byte a byte
 * (una vez removido el comentario que jsdelivr antepone a los archivos
 * que su propio sistema decide no re-minificar). SHA-256 del contenido
 * de la libreria (todo lo que sigue despues de este bloque de comentario):
 *   f387e5935730a6d9599a281986a176eafce5ebbe7514446222d21aa029e31dd5
 *
 * Esta copia NO se actualiza sola. Si en el futuro hace falta una version
 * mas nueva de supabase-js, hay que repetir el mismo proceso -- descargar
 * la version nueva desde al menos dos fuentes independientes, confirmar
 * que el hash coincide entre ambas, y recien entonces reemplazar este
 * archivo -- en vez de volver a apuntar a una URL con una etiqueta mutable.
 */
